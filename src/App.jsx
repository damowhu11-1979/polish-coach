...
import { Volume2, RotateCcw, Trophy, Zap, BookOpen, Timer, ListChecks, Flag, Target, Upload, Download, Settings as SettingsIcon, Mic, StopCircle, Wand2, Printer, Check } from "lucide-react";

/* ===== Inline minimal UI (no external '@/components/ui/*') ===== */
...
export function Button({ variant="default", size="default", className="", asChild=false, ...p }) {
  ...
  const Cmp=asChild?"span":"button"; return <Cmp className={cx(base,v,s,className)} {...p}/>;
}
...
function getVoicesPl(){ if(typeof window==="undefined") return []; const v=window.speechSynthesis?.getVoices?.()||[]; const pl=v.filter(x=>x.lang?.toLowerCase?.().startsWith("pl")); return pl.length?pl:v; }
...
export function PronounceBtn({ text, title="Pronounce (Polish)" }){ return <Button variant="ghost" size="icon" onClick={()=>speak(text)} title={title} aria-label={title}><Volume2 className="h-4 w-4"/></Button>; }
/* ===== end inline UI ===== */
...
const STARTER_DECK = [
  ...
  { id:"n1", pl:"jeden", en:"one", ipa:"ˈjɛ.dɛn", cat:"numbers" },
  { id:"n2", pl:"dwa", en:"two", ipa:"dva", cat:"numbers" },
  { id:"n3", pl:"trzy", en:"three", ipa:"t͡ʂɨ", cat:"numbers" },
  { id:"n4", pl:"cztery", en:"four", ipa:"ˈt͡ʂtɛ.rɨ", cat:"numbers" },
  { id:"n5", pl:"pięć", en:"five", ipa:"pjɛɲt͡ɕ", cat:"numbers" },
  { id:"n6", pl:"sześć", en:"six", ipa:"ʂɛɕt͡ɕ", cat:"numbers" },
  { id:"n7", pl:"siedem", en:"seven", ipa:"ˈɕɛ.dɛm", cat:"numbers" },
  { id:"n8", pl:"osiem", en:"eight", ipa:"ˈɔ.ɕɛm", cat:"numbers" },
  { id:"n9", pl:"dziewięć", en:"nine", ipa:"ˈd͡ʑɛ.vjɛɲt͡ɕ", cat:"numbers" },
  { id:"n10", pl:"dziesięć", en:"ten", ipa:"ˈd͡ʑɛ.ɕɛɲt͡ɕ", cat:"numbers" },
];
const LS_KEYS = { CARDS:"polishCoach.cards", STREAK:"polishCoach.streak", GOAL:"polishCoach.goal", DONE_TODAY:"polishCoach.doneToday", EXAM:"polishCoach.examProfile", EXAM_CFG:"polishCoach.examCfg", EXAM_BANK:"polishCoach.examBank", B1_CFG:"polishCoach.b1cfg", B1_HISTORY:"polishCoach.b1History", CIVICS_HISTORY:"polishCoach.civicsHistory" };
  ...
  return (
    <Card className="w-full max-w-xl">
      <CardHeader className="flex flex-row items-center justify-between">
        ...
        <div className="flex items-center gap-1"><Chip>{card.cat}</Chip><PronounceBtn text={card.pl} /></div>
      </CardHeader>
        ...
        <AnimatePresence mode="wait">
          {!revealed ? (
            <motion.div key="prompt" initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -6 }}>
              <Button className="w-full" onClick={() => setRevealed(true)}>Show meaning</Button>
            </motion.div>
          ) : (
            <motion.div key="answer" initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -6 }} className="text-center text-xl">
              {card.en}
            </motion.div>
          )}
        </AnimatePresence>
...
function MCQuiz({ deck, onComplete }) {
  ...
  const q=items[idx];
  const options=useMemo(()=>{ const pool=deck.filter(c=>c.id!==q.id); const d=sample(pool,Math.min(3,pool.length)); return shuffle([...d,q]); },[q,deck]);
  ...
  return (
    <Card className="max-w-xl w-full">
      <CardHeader><CardTitle className="flex items-center justify-between"><span>What does "{q.pl}" mean?</span><PronounceBtn text={q.pl} /></CardTitle></CardHeader>
      <CardContent className="space-y-3">
        {options.map((o) => (
          <Button key={o.id} variant="outline" className="w-full justify-start" onClick={() => next(o.id === q.id)}>{o.en}</Button>
        ))}
        <div className="text-sm text-gray-500">{idx + 1} / {items.length} — Score: {score}</div>
      </CardContent>
    </Card>
  );
}
...
function Grammar() {
  return (
    <div className="max-w-3xl space-y-6">
      <Card><CardHeader><CardTitle>Polish Sounds Quick Guide</CardTitle></CardHeader>
        <CardContent className="text-sm text-gray-600">
          <ul className="list-disc pl-5 space-y-1">
            <li><b>ą</b> /ɔ̃/, <b>ę</b> /ɛ̃/ (often denasalised word-finally).</li>
            <li><b>ł</b> ≈ “w”, <b>rz</b> ≈ French “j”, <b>sz</b> ≈ “sh”, <b>ch</b> /x/.</li>
          </ul>
        </CardContent>
      </Card>
    </div>
...
function useHistory(key){
  ...
  useEffect(()=>{safeSet(key,JSON.stringify(hist));},[key,hist]); return [hist,setHist];
}
...
function ExamQuestion({ q, index, total, selected, setSelected, revealed }) {
  ...
  return (<Card className="w-full">
    ...
    <CardContent className="space-y-2">
      ...
      {revealed && <div className="text-sm text-gray-600 pt-2"><b>Explanation:</b> {q.expl}</div>}
    </CardContent>
  </Card>);
}
...
function MCBlock({ items, onFinish, onScore }) {
  const [i,setI]=useState(0); const [sel,setSel]=useState(null); const [show,setShow]=useState(false); const [score,setScore]=useState(0); const q=items[i]; if(!q) return null;
  ...
  return (<Card className="w-full max-w-2xl"><CardHeader><CardTitle className="text-base">Pytanie {i+1}/{items.length} — {score} pkt</CardTitle></CardHeader>
    <CardContent className="space-y-2"><div className="text-lg">{q.q}</div>
      {q.opts.map((opt,idx)=> { const correct = idx===q.a, chosen = sel===idx; const variant = show ? (correct?"default":chosen?"destructive":"outline") : (chosen?"secondary":"outline"); return <Button key={idx} variant={variant} className="w-full justify-start" onClick={()=>!show && setSel(idx)}>{opt}</Button>; })}
      <div className="flex gap-2 pt-2"><Button onClick={()=>setShow(true)} disabled={show||sel==null}>Reveal</Button><Button onClick={()=>next(sel===q.a)} disabled={!show}>Next</Button></div>
    </CardContent></Card>);
}
function ExamBannerLite({ minutes, setMinutes, secondsLeft, setSecondsLeft, running, setRunning }) {
  ...
  return (<Card className="mb-3"><CardContent className="py-3 flex items-center gap-3"><Timer className="h-5 w-5" /><div className="text-sm w-24">{mins}:{secs.toString().padStart(2,"0")}</div>
    <div className="flex items-center gap-2 ml-auto"><Input type="number" className="w-20" min={1} value={minutes} onChange={(e)=>setMinutes(Math.max(1,Number(e.target.value)||minutes))}/><Button onClick={()=>{ setSecondsLeft(minutes*60); setRunning(true); }}>Start</Button><Button variant="secondary" onClick={()=>setRunning(false)}>Pause</Button><Button variant="outline" onClick={()=>{ setRunning(false); setSecondsLeft(minutes*60); }}>Reset</Button></div>
  </CardContent></Card>);
}
function B1Reading({ onSaveAttempt }) {
  const [setIdx,setSetIdx]=useState(0); const set=READ_SETS[setIdx];
  return (<div className="space-y-3">
    ...
    <Card className="max-w-3xl"><CardHeader><CardTitle>{set.title}</CardTitle></CardHeader><CardContent className="space-y-3"><p className="text-sm text-gray-600">{set.passage}</p></CardContent></Card>
...
function B1Listening({ onSaveAttempt }) {
  const [setIdx,setSetIdx]=useState(0); const set=LISTEN_SETS[setIdx]; const [playing,setPlaying]=useState(false);
  ...
  return (<div className="space-y-3">
    ...
    <Card className="max-w-3xl"><CardHeader><CardTitle>{set.title}</CardTitle></CardHeader><CardContent className="space-y-2"><div className="text-sm text-gray-600">Press to play once. Questions below.</div><Button onClick={play} disabled={playing}><Volume2 className="h-4 w-4 mr-1" />Play (TTS)</Button></CardContent></Card>
...
function B1Writing({ onSaveAttempt }) {
  ...
  const [idx,setIdx]=useState(0); const [text,setText]=useState(""); const p=PROMPTS[idx];
  const words=tokenize(text).length; const result=gradeWriting(text, p.min, p.max);
  const tips = [
  ...
  return (<div className="space-y-3 max-w-3xl">
    ...
    <Card><CardHeader><CardTitle>{p.title}</CardTitle></CardHeader><CardContent className="space-y-3">
      <div className="text-sm text-gray-600">{p.task}</div>
      <textarea value={text} onChange={(e)=>setText(e.target.value)} rows={12} className="w-full border rounded p-2" placeholder="Pisz tutaj..."/>
      <div className="text-xs text-gray-500">Words: {words} • Sentences: {result.sent} • Avg len: {result.avgLen}</div>
      <Card className="border"><CardContent className="py-3 space-y-2">
        ...
        {tips.length>0 && <div className="text-xs text-gray-600 pt-1">Tips: {tips.join(" · ")}</div>}
...
function B1Speaking({ onSaveAttempt }) {
  ...
  const [idx,setIdx]=useState(0); const card=CARDS[idx];
  const [rec,setRec]=useState(null); const [url,setUrl]=useState(""); const [startedAt,setStartedAt]=useState(0); const [duration,setDuration]=useState(0);
  const [transcript,setTranscript]=useState(""); const [recog,setRecog]=useState(null);
  async function startRec(){ if(rec) return;
    try{ const stream = await navigator.mediaDevices.getUserMedia({ audio:true });
      const m = new MediaRecorder(stream); const chunks=[]; m.ondataavailable = e => { if(e.data.size>0) chunks.push(e.data); };
      m.onstop = () => { const blob = new Blob(chunks,{type:"audio/webm"}); const u = URL.createObjectURL(blob); setUrl(u); stream.getTracks().forEach(t=>t.stop()); setDuration(Math.round((Date.now()-startedAt)/1000)); };
  ...
  function startTranscribe(){
    const SR = window.SpeechRecognition || window.webkitSpeechRecognition; if(!SR){ alert("SpeechRecognition not supported in this browser."); return; }
  ...
  return (<div className="space-y-3 max-w-3xl">
    ...
    <Card><CardHeader><CardTitle>{card.title}</CardTitle></CardHeader><CardContent className="space-y-3">
      <div className="text-sm text-gray-600">{card.prompt}</div>
      ...
      <Card className="border"><CardContent className="py-3 space-y-2">
        ...
        <div className="text-xs text-gray-600">Dur: {scoring.durationSec}s • Fillers≈ {Math.round(scoring.fill*100)}% • Unique≈ {Math.round(scoring.uniq*100)}%</div>
...
function B1Trainer({ onSaveAttempt }) {
  ...
  return (<div className="space-y-6">
    <Tabs value={tab} onValueChange={setTab}>
      <TabsList className="flex flex-wrap"><TabsTrigger value="reading">Reading</TabsTrigger><TabsTrigger value="listening">Listening</TabsTrigger><TabsTrigger value="writing">Writing</TabsTrigger><TabsTrigger value="speaking">Speaking</TabsTrigger><TabsTrigger value="settings">Settings</TabsTrigger></TabsList>
      <TabsContent value="reading" className="pt-4"><ExamBannerLite minutes={rm} setMinutes={setRm} secondsLeft={rs} setSecondsLeft={setRs} running={rr} setRunning={setRr} /><B1Reading onSaveAttempt={(r)=>onSaveAttempt?.(r)} /></TabsContent>
      <TabsContent value="listening" className="pt-4"><ExamBannerLite minutes={lm} setMinutes={setLm} secondsLeft={ls} setSecondsLeft={setLs} running={lr} setRunning={setLr} /><B1Listening onSaveAttempt={(r)=>onSaveAttempt?.(r)} /></TabsContent>
      <TabsContent value="writing" className="pt-4"><ExamBannerLite minutes={wm} setMinutes={setWm} secondsLeft={ws} setSecondsLeft={setWs} running={wr} setRunning={setWr} /><B1Writing onSaveAttempt={(r)=>onSaveAttempt?.(r)} /></TabsContent>
      <TabsContent value="speaking" className="pt-4"><B1Speaking onSaveAttempt={(r)=>onSaveAttempt?.(r)} /></TabsContent>
      <TabsContent value="settings" className="pt-4"><B1Settings /></TabsContent>
    </Tabs>
  </div>);
}
...
function ExamDrill({ bank, cat, onDone }) {
  ...
  const q=items[i]; if(!q) return <div className="text-sm text-gray-600">No questions.</div>;
...
function CitizenshipModule({ onSaveAttempt }) {
  ...
  const [profile,setProfile]=useExamProfile(); const [cfg,setCfg]=useExamCfg(); const [bank,setBank]=useExamBank();
  const [mode,setMode]=useState("mock"); const [cat,setCat]=useState(""); const [seed,setSeed]=useState(()=>Math.random()); const [submitted,setSubmitted]=useState(false);
  ...
  const [picks,setPicks]=useState(()=>Array(testItems.length).fill(null)); const [timeLeft,setTimeLeft]=useState(cfg.timeMin*60);
  ...
  function submit(){
    ...
    const got = testItems.reduce((n,q,i)=> n + (picks[i]===q.answer?1:0), 0); const total = testItems.length;
  ...
  function printMockSheet(){
    ...
    const html = `
      ...
      <p>Questions: ${items.length} • Time: ${cfg.timeMin} min</p>
      ${items.map((q,i)=>`
  ...
  const GuideView=(<div className="grid gap-4 max-w-3xl">
    <Card><CardHeader><CardTitle><Flag className="h-5 w-5 inline mr-2"/>Constitution</CardTitle></CardHeader><CardContent className="text-sm text-gray-600">Unitary parliamentary republic; Sejm/Senat; President (5y) + PM-led Council of Ministers.</CardContent></Card>
    <Card><CardHeader><CardTitle><BookOpen className="h-5 w-5 inline mr-2"/>History</CardTitle></CardHeader><CardContent className="text-sm text-gray-600">966; 1569; 1791; 1918; 1980–89; 2004 (EU).</CardContent></Card>
  </div>);
  ...
  return (
    <div className="space-y-6">
      <div className="grid md:grid-cols-4 gap-3">
        <Card><CardContent className="py-4"><div className="text-xs text-gray-600 mb-1">Overall Seen</div><div className="text-2xl font-semibold">{totalSeen}</div></CardContent></Card>
        <Card><CardContent className="py-4"><div className="text-xs text-gray-600 mb-1">Overall Correct</div><div className="text-2xl font-semibold">{totalOk}</div></CardContent></Card>
        <Card><CardContent className="py-4"><div className="text-xs text-gray-600 mb-1">Best Subject</div><div className="text-sm">{["constitution","history","geography","culture"].sort((a,b)=>pct(b)-pct(a))[0]||"—"}</div></CardContent></Card>
        <Card><CardContent className="py-4"><div className="text-xs text-gray-600 mb-1">Weakest Subject</div><div className="text-sm">{["constitution","history","geography","culture"].sort((a,b)=>pct(a)-pct(b))[0]||"—"}</div></CardContent></Card>
      </div>
...
/* ===== Analytics (simple sparkline-free, to avoid extra deps) ===== */
function Analytics({ b1History, civicsHistory, exportAll }) {
  ...
  return (
    <div className="space-y-3">
      <div className="flex gap-2 justify-end">
        <Button onClick={()=>exportAll("json")}><Download className="h-4 w-4 mr-1"/>Export JSON</Button>
        <Button variant="secondary" onClick={()=>exportAll("csv")}><Download className="h-4 w-4 mr-1"/>Export CSV</Button>
      </div>
      <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-3">
        <Block title="B1 Reading" rows={groups.reading}/>
        <Block title="B1 Listening" rows={groups.listening}/>
        <Block title="B1 Writing" rows={groups.writing}/>
        <Block title="B1 Speaking" rows={groups.speaking}/>
        <Block title="Civics Mock" rows={groups.civics}/>
      </div>
    </div>
  );
}
...
export default function App(){
  ...
  function exportAll(kind){
    if(kind==="json"){ const payload = { b1History, civicsHistory }; downloadFile("polish-coach-history.json", JSON.stringify(payload,null,2), "application/json"); }
    else { const rows = [ ...b1History.map(r=>({...r, kind:"B1"})), ...civicsHistory.map(r=>({...r, kind:"CIVICS"})), ]; downloadFile("polish-coach-history.csv", toCSV(rows), "text/csv"); }
  }
    ...
    <div className="min-h-screen w-full bg-gradient-to-b from-white to-slate-50">
      <div className="mx-auto max-w-6xl px-4 py-6">
        <header className="mb-6 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div><h1 className="text-3xl font-bold tracking-tight">🇵🇱 Polish Coach + Citizenship + B1</h1><p className="text-gray-600">Civics mock tests + B1 trainer with auto-scoring & analytics.</p></div>
        ...
        <Tabs value={tab} onValueChange={setTab}>
          ...
          <TabsContent value="learn" className="pt-4"><div className="flex flex-col items-center gap-4"><Flashcard card={active} onGrade={grade}/><p className="text-sm text-gray-600">Speaker = native-like browser TTS.</p></div></TabsContent>
          <TabsContent value="practice" className="pt-4"><MCQuiz deck={deck} onComplete={(score,total)=>{ alert(`Quiz complete! ${score}/${total}`); setDone(d=>d+total); setTab("learn"); }}/></TabsContent>
          <TabsContent value="vocab" className="pt-4"><div className="flex gap-2 mb-4 flex-wrap"><Chip key="all" active={!filterCat} onClick={()=>setFilterCat("")}>all</Chip>{categories.map(cat=>(<Chip key={cat} active={filterCat===cat} onClick={()=>setFilterCat(cat)}>{cat}</Chip>))}</div><VocabTable deck={deck} filter={filterCat}/></TabsContent>
          ...
          <TabsContent value="analytics" className="pt-4"><Analytics b1History={b1History} civicsHistory={civicsHistory} exportAll={exportAll}/></TabsContent>
          ...
          <TabsContent value="settings" className="pt-4">
            <Card className="max-w-xl"><CardHeader><CardTitle>Settings</CardTitle></CardHeader>
              <CardContent className="space-y-4">
                ...
                <div className="text-sm text-gray-600">Progress lives only in your browser.</div>
              </CardContent></Card>
          </TabsContent>
        </Tabs>

        <footer className="mt-10 text-center text-xs text-gray-600">Tip: Export attempts in <b>Analytics</b>. Print sheets in <b>B1 Settings</b> or <b>Citizenship Settings</b>.</footer>
      </div>
    </div>
  );
}
